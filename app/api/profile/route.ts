...
export async function PATCH(req: Request) {
  ...
  let body: { name?: string; firstName?: string; familyName?: string; bio?: string };
  try {
    body = await req.json();
  } catch {
  ...
  await pool.query(
    `UPDATE users SET name = $1, first_name = $2, family_name = $3, bio = $4 WHERE id = $5`,
    [name || null, firstName || null, familyName || null, bio || null, user.id]
  );
